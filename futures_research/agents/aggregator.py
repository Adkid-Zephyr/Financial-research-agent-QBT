...
async def aggregate_node(state: Dict[str, Any], runtime: RuntimeContext) -> Dict[str, Any]:
    ...
    active_source_types: List[str] = [
        source_type for source_type in configured_source_types if runtime.data_source_registry.has(source_type)
    ]
    ...
    verified_facts = [item for payload in payloads for item in payload.highlights]
    data_gaps = [
